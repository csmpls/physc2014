...
Specifically, we find that our quantization method decreases the computational expense of EEG-based calibration (from 18 ms to 2ms for SVM training time) without a significant detriment to accuracy and, using quantized data, our opportunistic user calibration strategy achieves an average of 88.3\% accuracy across all subjects. All subjects require under fifteen minutes of calibration time, and 86.6\% of these subjects require five minutes or fewer.

The conclusions to be drawn from this study are limited in a few regards. First, calibration and classification are performed offline, so factors involving the user interface (such as feedback) are not taken into account. We cannot be sure, for instance, that our findings based on the splicing of 10-second-long recorded data will persist when a system solicits recordings of only a second or under. Furthermore, a few of the tasks (e.g., the color task) relied on exogenous stimuli, which may be impractical in naturalistic settings for ergonomic reasons. 
